...
sap.ui.core.routing.Router.extend("ui5app.MyRouter", {
  ...
  processConventionNavRoute: function(sName, oParameters){
    ...
    var hashPartsLen = hashParts.length || 0;
    var conventionViewFiles;

    //if(hashPartsLen>0 && $.isNumeric(hashParts[hashPartsLen-1])){
    if(!!oParameters && !!oParameters.id){
      VCdir = currPath;
      conventionViewFiles = ['Detail'];
    }else{
      VCdir = sName;
      conventionViewFiles = ['Master','Main','Page','v'];
    }
